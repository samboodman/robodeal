    ...
    configureServer(server) {
      server.middlewares.use('/api/realtime-call', async (request, response) => {
        ...
        try {
          ...
          if (typeof requestBody.sdp !== 'string' || requestBody.sdp.length === 0) {
            response.statusCode = 400;
            response.end(JSON.stringify({ error: 'A WebRTC offer is required.' }));
            return;
          }
...
export default defineConfig(({ mode }) => {
  ...
  return {
    plugins: [localRealtimeApi(env.OPENAI_API_KEY)],
  };
});
