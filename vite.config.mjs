...
function localWordsToNumberApi() {
  return {
    ...
    configureServer(server) {
      server.middlewares.use('/api/words-to-number', async (request, response) => {
        response.setHeader('Content-Type', 'application/json; charset=utf-8');
        if (request.method !== 'POST') {
          response.statusCode = 405;
          response.end(JSON.stringify({ error: 'Use POST for words-to-number requests.' }));
          return;
        }
        try {
          ...
          response.end(JSON.stringify({ error: 'The words-to-number request was invalid.' }));
        }
      });
    },
  };
