...
function chunkedAudioResponse(chunks) {
  return new Response(
    new ReadableStream({
      start(controller) {
...
test('client VAD commits complete commands faster while allowing incomplete speech more time', () => {
  ...
  agent.sampleVoiceActivity(1200);
  inputLevel = 0;
  ...
  inputLevel = 0;
  agent.sampleVoiceActivity(2849);
  assert.equal(sent.length, 1);
  agent.sampleVoiceActivity(2850);
