...
export async function handleVoiceApi(body, apiKey) {
  if (!apiKey) {return errorResult(500, 'The server is missing OPENAI_API_KEY.');}
  ...
  if (body.action === 'transcribe') {
    ...
    if (audioBytes.length > 12_000_000) {return errorResult(413, 'The audio turn is too large.');}
    const form = new FormData();
    form.append('model', 'gpt-transcribe');
    form.append('file', new Blob([audioBytes], { type: body.mimeType || 'audio/webm' }), body.fileName || 'audio.webm');
    if (body.prompt) {form.append('prompt', String(body.prompt).slice(0, 4_000));}
  ...
  if (body.action === 'speech') {
    ...
    return {
      ...
      body: new Uint8Array(await openAIResponse.arrayBuffer()),
    };
  }
